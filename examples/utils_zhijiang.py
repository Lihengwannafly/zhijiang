...
class ZhiJiangProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        label_list = []
        bio_list = ['B', 'I']
        cate_list = ['baozhuang', 'chengfen', 'chicun', 'fuwu', 'gongxiao', 'jiage', 'qiwei', 'shiyongtiyan', 'wuliu',
                     'xinxiandu', 'zhenwei', 'zhengti', 'qita']
...
def get_metrics(data_dir='/data/share/zhanghaipeng/data/zhijiang/', gold_result_file='TEST/gold_Result.csv',
                pred_result_file='TEST/pred_Result_.csv'):
            ...
            for j in range(gold_data.shape[0]):
                ...
                if pred_sample['OpinionTerms'] == gold_sample['OpinionTerms']:
                    if pred_sample['AspectTerms'] == gold_sample['AspectTerms']:
                        s1 = 1
                    if pred_sample['Polarities'] == gold_sample['Polarities'] and pred_sample['Categories'] == \
                            gold_sample['Categories']:
                        s2 = 1
                    s = s1 * 0.6 + s2 * 0.4
                s_list.append(s)
...
def get_labels(data_dir='/data/电商评论观点挖掘/', train_review_file='TRAIN/Train_reviews.csv',
               train_result_file='TRAIN/Train_labels.csv', train_label_file='TRAIN/Label.csv'):
...
def split_labels(data_dir='/data/电商评论观点挖掘/', label_file='TRAIN/Label.csv',
                 train_file='TRAIN/train.csv', test_file='TRAIN/test.csv'):
    review = pd.read_csv(data_dir + label_file, sep='\t')
    train, test = train_test_split(review, train_size=0.8, random_state=64)
    train.to_csv(data_dir + train_file, index=False, sep="\t")
    test.to_csv(data_dir + test_file, index=False, sep="\t")
